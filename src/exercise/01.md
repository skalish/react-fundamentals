...
JavaScript can be run in an html document to make changes to the document itself via the DOM API.  This should be done in a "script" element, which can be of type "module", making this export/importable.

A div element can be created with `document.createElement('div')`. Its attributes can be set with the `setAttribute` method or by assignment: `divObject.id = 'root'`.

Elements can be set as children of other elements with the `append` method.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=01%3A%20Basic%20JavaScript-rendered%20Hello%20World&em=samuel.kalish%40tamr.com).
